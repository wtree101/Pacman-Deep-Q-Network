...
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getWeights(self, gameState, action,features):
    #要小心不要为了追人跑出防守距离。
    weights_dict = {'numInvaders': -1000, 'onDefense': 500, 'invaderDistance': -20, 'stop': -100,
    'enemyDistance':-5}
...
###########1 可走的路少 -- 但是感觉很难实现的
    ...
    enemies = [successor.getAgentState(i) for i in self.getOpponents(successor)]
    defenders = [a for a in enemies if (not a.isPacman) and a.getPosition() != None and a.scaredTimer<=8] #被恐吓的幽灵不算defenders
...
############################构造性特征
  ...
  def getWeights_offensive(self, gameState, action,features):
    ...
    'Capsule_attract':-6,'stop': -200} #-300是为了如果有机会吃掉它，不因为吃了之后胶囊的惩罚而止步不前
    #危险状态
    ...
    if (score_now + myState.numCarrying - features['invadercarrying']>0) and myState.numCarrying>10: #考虑返回,但要保证分界点处可以进行——要吃掉最后一个豆豆
      weights_dict['dividesDistance'] = -15 #一定要返回了，比食物吸引力大
    # #被追逐状态
