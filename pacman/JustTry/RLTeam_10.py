...
class RLagent(DefensiveReflexAgent):
    def __init__(self,index,numTraining=0): #再写了一个重构函数
       ...
       # print('RL',self.numTraining)
        ...
        self.maxTunnel = 0
        self.weights = util.Counter({'eat_food': 0.5160101459387367, 'distanceToFood': -39.550261982857876, 'check_died': -1.9507860520744682, 'divides_attract': -29.278496512215572, 'Capsule_attract': -4.08727502447775, 'stop': -5.0, 'CornerAvoid': 0.10808655548588508, 'bias': 1.9532882349885847, 'defendersAvoid': 0.3406171965324117})
    ...
    def observationFunction(self, gameState):#重写这个观察函数，改成RL版本，会被调用
        ...
        if not self.lastState is None:
           ...
           # score_change = gameState.getScore() - self.lastState.getScore() #排除对面的影响
            score_change = 0
            if (myState.getPosition()in self.divides) and (last_myState.numCarrying>0):
                score_change = last_myState.numCarrying
    ...
    def update(self, state, action, nextState, reward):
        ...
        if (features['inTunnel']==1):
            print(state.getAgentState(self.index))
            print(action)
            print(self.weights)
            print(features)
            print('reward:',reward)
            print('nextV:',self.getValue(nextState))
            print('Qnew',reward + self.discount*self.getValue(nextState))
            print('Qold',self.getQValue(state,action))
      #  print('Scaling:' )
      #   print(features)
    ...
    #################指示变量
       ...
       # features['realScore'] = self.getScore(successor) #权1.0 realscore是环境的真实反馈，我觉得这里不需要了。
        ...
        if myPos in Capsules_old:
            data['Capsule_eaten'] = 1
        else:
            data['Capsule_eaten'] = 0
...
#先做指示特征
      #  if (features['Capsule_eaten']==0):
        ...
        # else: #为此而特地改写特征，在吃掉胶囊时先不要把敌人清走，而是保留旧的敌人，这样转移时可以体现出这个状态的优越处
        ...
        # if (features['Capsule_eaten']==1):#如果吃掉了胶囊其实是不会死的
        #     features['check_died'] = 0
        ...
        if data['defenders']>0:
            ...
            if features['inTunnel']:
             # features['distanceOutofDeadcorner'] /= (walls.width * walls.height)
              features['CornerAvoid'] = 1/(1+math.exp(-(2*data['distanceOutofDeadcorner']-data['defendersDistance'])) )#负权
              #print(features['CornerAvoid'])
    ...
    #############################吃胶囊
       ...
       # features['dividesDistance'] /= (walls.width * walls.height) #都要规范化
            ...
            if data['defenders']!=0:  #有防守者才要考虑的东西 <=5 此时防守者距离也是存在的
                ...
                if features['eat_food']==0 and data['Capsule_eaten']==0: #eat_food的时候，不考虑Capsule距离的惩罚
                    features['Capsule_attract'] = data['CapsulesDistance_opponent']
                    features['Capsule_attract'] *= features['defendersAvoid']
            # features['Capsule_attract'] /= (walls.width * walls.height)

    #############################路口策略   
        if len(defenders)>0:     
            features['divides_attract'] = data['dividesDistance']*data['numCarrying']*features['defendersAvoid']
            # if (features['divides_attract']>0):
            #   print('divides_attract',features['divides_attract'])
    ...
    def ScalingFeatures(self,walls,features):
        ...
        # features['CornerAvoid'] /= (walls.width*walls.height) #感觉特征量级太大训练时就老爆炸。。
        # features['defendersAvoid'] /= (walls.width*walls.height)
