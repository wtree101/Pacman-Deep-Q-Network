...
#############################吃豆豆
    ...
    if features['defenders']>0:
      for deadcorner in self.deadCorners.keys():
        ...
        if (dis_out*2+1<=features['defendersDistance']):
...
############################构造性特征
  ...
  def getWeights_offensive(self, gameState, action,features):
    ...
    weights_dict = {'successorScore': 200, 'distanceToFood': -10,'check_died':-500,'check_trapped':-300,'defendersDistance':1,
    ,'dividesDistance':-1,'CapsulesDistance_opponent':-1,'numCapsules_opponent':-200,
    'Capsule_attract':-6,'possibly_trapped':-100,'stop': -50} #-300是为了如果有机会吃掉它，不因为吃了之后胶囊的惩罚而止步不前
    #危险状态
    if features['defenders']>1 and (score_now + myState.numCarrying>0) and myState.numCarrying>0: #考虑返回,但要保证分界点处可以进行——要吃掉最后一个豆豆
      weights_dict['dividesDistance'] = -200 #强行返回，不会路过别的
    # #被追逐状态
