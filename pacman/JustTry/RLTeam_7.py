...
class RLagent(DefensiveReflexAgent):
    def __init__(self,index,numTraining=0): #再写了一个重构函数
       ...
       # print('RL',self.numTraining)
        ...
        self.maxTunnel = 0
        self.weights = util.Counter({'eat_food': 0.6766792171082697, 'distanceToFood': -11.743359237305329, 'check_died': -1.555594201067353, 'divides_attract': -5.053999598491741, 'Capsule_attract': -0.004616110096526459, 'stop': -0.023444466981070142,  'CornerAvoid': -1.6045720539328574, 'bias': 1.1216024914115372, 'defendersAvoid': -1.001901978829867})
        ...
        if data['defenders']>0:
            ...
            if data['inTunnel']:
             # features['distanceOutofDeadcorner'] /= (walls.width * walls.height)
              features['CornerAvoid'] = math.exp((2*data['distanceOutofDeadcorner']-data['defendersDistance'])) #负权
