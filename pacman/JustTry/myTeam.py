...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def P_deadcorners(self,pos,walls):
    ...
    if tunnel_len>0:
      self.deadCorners[start] = tunnel_len
...
###########1 可走的路少 -- 但是感觉很难实现的
    ...
    if len(defenders) > 0: #如果等于0，这个特征就不存在了。
      dists = [self.getMazeDistance(myPos, a.getPosition()) for a in defenders if a.scaredTimer<=5 ] #被恐吓的幽灵不需要害怕
      if (len(dists)>0):
...
############################构造性特征
  ...
  def getWeights_offensive(self, gameState, action,features):
    ...
    weights_dict = {'realScore':500,'successorScore': 200, 'distanceToFood': -5,'check_died':-1000,'check_trapped':-300,'defendersDistance':1
